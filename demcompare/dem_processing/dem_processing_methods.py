...
class RefCurvature(DemProcessingTemplate):
    ...
    def compute_curvature_filtering(
        self, dem: xr.Dataset, k: float, rep: bool
    ) -> xr.Dataset:
        """
        ...
        :param dem: dem xr.DataSet containing :
                ...
                - classification_layer_masks : 3D (row, col, indicator)
                  xr.DataArray
        :type dem: xr.Dataset
        :param k: DEM filter intensity. Should be close to 1.
        :type k: float
        :param rep: if true the image is replicated by x4
        :type rep: bool
    ...
    def process_dem(
        ...
        return self.compute_curvature_filtering(dem_1, 0.9, True)
