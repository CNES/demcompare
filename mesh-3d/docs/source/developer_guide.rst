...
.. code-block:: bash
    ...
    ## Update to real URL !
    git clone https://github.com/CNES/mesh_3d
    cd mesh_3d
    make install
...
* **Factorization**: Factorize the code as much as possible. The command line tools shall only include the main workflow and rely on the **mesh_3d** python modules.
...
mesh_3d includes a set of tests executed with `pytest <https://docs.pytest.org/>`_ tool.
...
It launches the unit tests present in `mesh_3d/tests/test_mesh_3d.py` displaying the traces generated by the tests and the tests code coverage level.

During the tests execution, mesh_3d will write the output data in a */tmp* directory.

Documentation
*************

mesh_3d documentation can be generated with following command:
...
This installs the pre-commit hook in `.git/hooks/pre-commit` and `.git/hooks/pre-push` from `.pre-commit-config.yaml <https://raw.githubusercontent.com/CNES/mesh_3d/master/.pre-commit-config.yaml>`_ file configuration.
...
.. code-block:: console
  ...
  $ pre-commit run --files mesh_3d/__init__.py   # Run all hooks on one file
...
``isort`` configuration is done in `.pyproject.toml <https://raw.githubusercontent.com/CNES/mesh_3d/master/pyproject.toml>`_
`Isort`_ manual usage examples:

.. code-block:: console

    $ cd mesh_3d_HOME
    $ isort --check mesh_3d tests  # Check code with isort, does nothing
    $ isort --diff mesh_3d tests   # Show isort diff modifications
    $ isort mesh_3d tests          # Apply modifications
...
``black`` configuration is done in `.pyproject.toml <https://raw.githubusercontent.com/CNES/mesh_3d/master/pyproject.toml>`_
...
.. code-block:: console
    ...
    $ cd mesh_3d_HOME
    $ black --check mesh_3d tests  # Check code with black with no modifications
    $ black --diff mesh_3d tests   # Show black diff modifications
    $ black mesh_3d tests          # Apply modifications

Flake8
------
`Flake8`_ is a command-line utility for enforcing style consistency across Python projects. By default it includes lint checks provided by the PyFlakes project, PEP-0008 inspired style checks provided by the PyCodeStyle project, and McCabe complexity checking provided by the McCabe project. It will also run third-party extensions if they are found and installed.

``flake8`` configuration is done in `setup.cfg <https://raw.githubusercontent.com/CNES/mesh_3d/master/setup.cfg>`_
...
.. code-block:: console
  ...
  $ cd mesh_3d_HOME
  $ flake8 mesh_3d tests           # Run all flake8 tests
...
``pylint`` configuration is done in dedicated `.pylintrc <https://raw.githubusercontent.com/CNES/mesh_3d/master/.pylintrc>`_ file.
...
.. code-block:: console
  ...
  $ cd mesh_3d_HOME
  $ pylint tests mesh_3d       # Run all pylint tests
...
Any proven or suspected malfunction should be traced in a bug report, the latter being an issue in the **mesh_3d** github repository.
...
**mesh_3d**'s Classical workflow is :
 - Check Licence and sign :ref:`contribution_license_agreement` (Individual or Corporate)
 - Create an issue (or begin from an existing one)
 - Create a Merge Request from the issue: a MR is created accordingly with *WIP:*, *Closes xx* and associated *xx-name-issue* branch
 - Modify **mesh_3d**'s code from a local working directory or from the forge (less possibilities)
...
**mesh_3d** requires that contributors sign out a `Contributor LicenseAgreement <https://en.wikipedia.org/wiki/Contributor_License_Agreement>`_.
...
`Individual Contributor LicensingAgreement <https://github.com/CNES/mesh_3d/blob/master/docs/source/CLA/ICLA-mesh_3d.doc>`_ (ICLA) form and a
`Corporate Contributor Licensing Agreement <https://github.com/CNES/mesh_3d/blob/master/docs/source/CLA/CCLA-mesh_3d.doc>`_ (CCLA) form if you are
...
modified source files and also added to the `AUTHORS.md file <https://github.com/CNES/mesh_3d/blob/master/AUTHORS.md>`_.
